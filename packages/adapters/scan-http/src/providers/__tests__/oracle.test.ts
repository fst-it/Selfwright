...
const baseTarget: ScanTarget = {
  company: "AcmeBank",
  provider: "oracle",
  careersUrl: "https://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001",
};
...
describe("oracleProvider.detect", () => {
  it("returns listing URL from a careersUrl with /sites/{site}/ in path", () => {
    ...
    expect(result?.url).toContain("jpmc.fa.oraclecloud.com");
  });
  ...
  it("returns null when careersUrl has no /sites/ path segment", () => {
    const target: ScanTarget = {
      ...baseTarget,
      careersUrl: "https://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en",
    };
    expect(oracleProvider.detect(target)).toBeNull();
  });
  ...
  it("returns null for an http:// careersUrl (must be https)", () => {
    const target: ScanTarget = {
      ...baseTarget,
      careersUrl: "http://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001",
    };
    expect(oracleProvider.detect(target)).toBeNull();
  });
  ...
  it("rejects site number with injection chars (semicolon)", () => {
    const target: ScanTarget = {
      ...baseTarget,
      careersUrl:
        "https://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001%3BevildParam%3Dx",
    };
    expect(oracleProvider.detect(target)).toBeNull();
  });
});
...
describe("oracleProvider.fetch", () => {
  it("maps Oracle items to RawPosting", async () => {
    ...
    expect(result[0]?.url).toContain("jpmc.fa.oraclecloud.com");
  });
  ...
  it("constructs posting URL with host and siteNumber from careersUrl", async () => {
    ...
    expect(url).toBe(
      "https://jpmc.fa.oraclecloud.com/hcmUI/CandidateExperience/en/sites/CX_1001/job/210582049",
    );
  });
